...
if __name__ == '__main__':
    ...
    PSET['t_ref'] = ps.ParameterRange(np.linspace(0.2, 3.8, 19))
    PSET['simtime'] = 3000.

    # set up directory structure
    savefolder = os.path.join('./lfp_simulations_varying_t_ref/')
    ...
    for i, paramset in enumerate(PSET.iter_inner()):
        ...
        with open(os.path.join(savefolder, 'id_parameters.txt'), 'a') as f:
            f.write(ps_id + '\n')
            f.write('%.3f, %.3f, %.3f, %.3f'%(paramset['eta'], paramset['g'], paramset['J'], paramset['t_ref']) + '\n')
