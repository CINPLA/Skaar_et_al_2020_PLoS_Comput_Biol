...
if __name__ == '__main__':
    ...
    PSET = NEST_PSET.copy()
    N_sims = 50000
    g=np.random.uniform(4.5, 6.0, N_sims)
    eta=np.random.uniform(1.5, 3.0, N_sims)
    J = np.random.uniform(0.1, 0.25, N_sims) # postsynaptic amplitude in mV
    varying_parameters = np.array([eta, g, J]).T


    # set up directory structure
    savefolder = os.path.join('./lfp_simulations_small/')
